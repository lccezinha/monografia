...
Com a aplicação devidamente configura, é possível iniciar o desenvolvimento.
...
ele apenas irá informar que a especificação não ainda não foi escrita, conforme a
...
crie-se um arquivo \textit{Ruby} no diretório \textit{features/stepdefinitions} contendo todos os
...
Todos os arquivos contidos no diretório \textit{features/stepdefinitions} são os arquivos que farão
...
é preciso iniciar o desenvolvimento desses passos, utilizando o códigos reais da aplicação.
...
através do comando \textit{bundle exec cucumber} e verificar que todos os passos estão falhando. Dessa forma
terminamos o primeiro passo no ciclo do BDD, onde criamos uma especificação que falha por não haver
nenhum código de execução na aplicação, agora é possível iniciar o desenvolvimento e entrar na segunda
parte do ciclo do BDD e iniciar a codificação utilizando TDD.
