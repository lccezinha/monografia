...
\noindent O uso de teste no desenvolvimento de \textit{software} proporciona um
código de melhor qualidade, confiança e segurança.
O TDD ajuda o desenvolvedor a ter um código melhor e mais simples,
enquanto que o BDD ajuda na validação de uma especificação de funcionalidade.
Esse trabalho tem como objetivo mostrar, através de um referencial teórico
e um estudo prático, como a utilização de técnicas de TDD e BDD
podem ajudar equipes ágeis a chegar nesse objetivo.
\begin{keywords}
...
% \noindent para não criar parágrafo (Retirar se existir parágrafo)
\noindent The use of test in software development enables better code in terms of quality,
confidence and security. TDD helps the developer to write a better and simple code,
whereas the BDD helps through the validation of a given specification's feature.
This work aims to show, through theorical references and as a study guide,
how the use of TDD and BDD techniques may help agile teams to achieve this goal.
\begin{keywords}
...
para garantir que o que foi codificado está funcionando e também irá suprir a necessidade do cliente
\cite{Pressman} \cite{Beck}.
...
nova linha de código adicionada no projeto pode fazer com que todo o \textit{software} pare de funcionar
\cite{TDD:Aniche}.
...
Porém o problema de muitas metologias é a burocrácia. Essas metodologias são focadas em documentação e gerenciamento rígido do projeto, muitas vezes acabam sendo vistas como um grande atraso para o projeto, já que a grande maioria
...
Segundo \cite{LivroScrumCasaDoCodigo} dentro do \textit{Scrum} existem três papéis definidos: o Time de desenvolvimento, \textit{Scrum Master} e o \textit{Product Owner}.
...
\begin{itemize}
  ...
  onde o \textit{Product Owner} mostra quais as atividades de maior prioridade naquele momento
...
Ao longo dos anos a indústria de \textit{software} acumula algumas grandes falhas, citadas por
\cite{FalhasDeSoftware:PedroCosta} e \cite{FalhasDeSoftware:ProfissionaisTI} :
...
irá criar uma base sólida de testes de unidade ao \textit{software}
...
\textit{software} para após isso desenvolver o código necessário que irá solucionar o cenário desse comportamento, dessa forma pode-se validar se de fato o código criado irá atender
...
escrever os cenários de teste juntas, já que a prática utiliza uma Linguagem Ubíqua na descrição dos seus cenários
\cite{BDD:CasaDoCodigo} \cite{BDD:Soares}.
...
\begin{itemize}
  ...
  \item Na quinta e última etapa do ciclo, após a codificação e refatoração é necessário
...
possível escrever especificações executáveis do ponto de vista do usuário.
...
tem como grande vantagem ser leve, possibilitar um desenvolvimento flexível e rápido
...
O estudo experimental proposto, será uma aplicação que dado o valor do salário bruto deverá retornar
ao usuário o valor de seu salário líquido e também retornar os valores do IRRF e INSS,
conforme Tabela \ref{tabela_1} do Imposto de Renda (IR) e Tabela \ref{tabela_2} referente ao Instituto Nacional do Seguro Social (INSS).
...
\begin{figure}[hbt!]
  \begin{center}
    ...
    \caption{Criando primeiro teste unitário que irá falhar}
    \label{figura_17}
    Autoria Própria
  \end{center}
\end{figure}
...
\begin{figure}[hbt!]
  \begin{center}
    ...
    \caption{Executando testes unitários para a classe \textit{SalaryCalculator}}
    \label{figura_18}
    Autoria Própria
  \end{center}
\end{figure}
...
\begin{figure}[hbt!]
  \begin{center}
    ...
    \caption{Executando testes unitários, testes funcionando corretamente}
    \label{figura_20}
    Autoria Própria
  \end{center}
\end{figure}
...
Na etapa seguinte é preciso realizar a codificação mínima do código para que todos os testes passem. A
codificação é vista na Figura \ref{figura_23}.

\clearpage

\begin{figure}[h!]
  \begin{center}
    \epsfig{file=images/app-13.png}
    \caption{Criando código mínimo para que os testes passem}
    \label{figura_23}
    Autoria Própria
  \end{center}
\end{figure}

No próximo passo é preciso executar novamente os testes unitários referentes na Figura \ref{figura_21}
e como a codificação mínima foi feita todos os testes passam, como visto na Figura \ref{figura_24}.

\begin{figure}[hbt!]
  \begin{center}
    \epsfig{file=images/app-14.png}
    \caption{Executando novamente os testes unitários}
    \label{figura_24}
    Autoria Própria
  \end{center}
\end{figure}

Agora seguindo para a próxima etapa é preciso realizar refatoração do código, já que o código
inicial era apenas para sair do estado de vermelho e atingir o estado verde. Nessa quarta etapa do
processo de BDD é preciso entrar no processo de refatoração dentro do TDD e também validar se os testes unitários ainda
passam com a nova implementação, a nova implementação é vista na Figura \ref{figura_25}.

\begin{figure}[h!]
  \begin{center}
    \epsfig{file=images/app-15.png}
    \caption{Reescrevendo o código da classe \textit{SalaryCalculator}}
    \label{figura_25}
    Autoria Própria
  \end{center}
\end{figure}

Para validar o final do ciclo do TDD é preciso executar novamente os testes unitários da Figura \ref{figura_21}
e verificar que todos ainda passam, ou seja, a nova implementação além de melhor continua garantindo o funcionamento
do código, conforme visto na Figura \ref{figura_26}.
...
\begin{figure}[hbt!]
  \begin{center}
    \epsfig{file=images/app-14.png}
    \caption{Executando novamente os testes unitários, código continua funcionando após a refatoração}
    \label{figura_26}
    Autoria Própria
  \end{center}
\end{figure}

Com esses passos se encerra o ciclo do TDD e para finalizar o ciclo do BDD é preciso executar novamente a especificação
escrita na Figura \ref{figura_12}. Na Figura \ref{figura_27} a execução da especificação mostra que o ciclo do BDD foi completado
pois a especificação que antes era falha agora está executando corretamente, garantindo a funcionalidade da aplicação.

\begin{figure}[hbt!]
  \begin{center}
    \epsfig{file=images/app-17.png}
    \caption{Executando a especificação, código funcionando corretamente}
    \label{figura_27}
    Autoria Própria
  \end{center}
\end{figure}

Com a realização do ciclo completa e a garantia que a funcionalidade está correta, mostra-se que o ciclo do BDD foi
concluído com sucesso.

\chapter{Resultados e Discussões}

Esse capítulo visa discutir sobre os resultados obtidos após a implementação do estudo experimental com base
no referencial teórico descrito ao longo do trabalho.

\section{Funcionamento da aplicação}

Para sair do cenário apenas de testes foi criada uma aplicação \textit{web} simples,
utilizando o \textit{framework} \textit{Sinatra}, onde o usuário irá informar o seu salário bruto e a aplicação
irá retornar os valores corretos referentes ao IRRF, INSS e salário líquido.

A execução da aplicação é vista na Figura \ref{figura_28}.

\begin{figure}[h!]
  \begin{center}
    \epsfig{file=images/app-16.png}
    \caption{Executando a aplicação no browser}
    \label{figura_28}
    Autoria Própria
  \end{center}
\end{figure}

Com a execução da aplicação \textit{web} funcionando corretamente,
foi possível provar que os testes unitários e a especificação ajudaram a garantir
o correto funcionamento do \textit{software}.

\section{Documentação Viva}

A utilização do \textit{Cucumber} para criação da especificação, além de servir para integrar equipes, pois a mesma
pode ser escrita em português, também irá servir como uma documentação viva do \textit{software}.

Documentação de \textit{software} é um problema pois muitas equipes por vezes não documentam, ou quando fazem,
a documentação fica ultrapassada. A utilização de BDD aliado a ferramenta \textit{Cucumber} pode ajudar a solucionar
os problemas relacionados a documentação, pois a especificação sempre será atualizada juntamente com os testes
e o código da aplicação.

\chapter{Considerações Finais}

O capítulo visa apresentar as considerações finais e propor trabalhos futuros sobre o tema abordado.

\section{Conclusão}

Com base no estudo experimental realizado, foi possível observar a importância da inclusão
da abordagem de testes no processo de desenvolvimento de \textit{software}, principalmente por equipes
ágeis que utilizam XP ou \textit{Scrum}, pois essas metodologias dão espaço ao uso dessa técnica.

O uso de TDD no processo de desenvolvimento foi de fundamental importância para a criação
de um código mais simples, focado na solução e enxuto. O \textit{feedback} rápido
proporcionado pelo TDD provou-se fundamental para descoberta e correção de erros.

Já a aplicação do BDD foi importante pois com a escrita da especificação da funcionalidade
seu entendimento foi mais fácil e a própria especificação se tornou uma documentação viva do \textit{software}
pois ela sempre será atualizada a medida que o \textit{software} irá crescer ou ser alterado.

Embora a aplicação desenvolvida tenha sido simples, o uso das técnicas de TDD e BDD mostraram
ser eficientes e podem ser introduzidas em aplicações reais.

Dessa forma o trabalho foi concluído com sucesso e foi possível notar as vantagens da abordagem
do TDD e BDD no processo de desenvolvimento de \textit{software}. Além de se criar uma aplicação
mais confiável, segura e de melhor qualidade, também é possível gerar uma documentação e garantir
uma melhor integração entre equipes de diferentes áreas.

\section{Trabalho Futuros}

A continuação desse trabalho sugere um estudo melhor elaborado sobre como introduzir práticas
de testes unitários e de aceitação em equipes que não tem hábito de utilizar
testes e nem mesmo metodologias ágeis.

Além disso, uma pesquisa de levantamento de dados sobre como equipes ágeis fazem o uso dessas técnicas
e qual a real vantagem essas equipes observam nessa abordagem.
